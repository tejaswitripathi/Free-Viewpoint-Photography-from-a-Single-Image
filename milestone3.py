...
from utils import *
from InteractiveViewSplatter import *


DEFAULT_S3_URI = (
    "s3://tejas-blender-bucket/defocus-dataset-multiview/"
    "bedroom/dataset/img_00000_f2.799999952316284_fl50.0_fd2.77/source/"
)
...
def main():
    parser = argparse.ArgumentParser(description="Milestone 3: CPU Gaussian splat renderer.")
    parser.add_argument("--s3-uri", default=DEFAULT_S3_URI, help="S3 prefix containing rgb.png, depth EXR, metadata.json")
    parser.add_argument("--local-dir", default=None, help="Use an existing local sample folder instead of S3")
    parser.add_argument("--out-dir", default="milestone3_outputs", help="Output directory for renders and gaussians.npz")
    parser.add_argument("--stride", type=int, default=2, help="Backprojection stride. 1 is full-res but slow.")
    parser.add_argument("--base-world-scale", type=float, default=0.004)
    parser.add_argument("--min-radius-px", type=float, default=1.5)
    parser.add_argument("--max-radius-px", type=float, default=7.0)
    parser.add_argument("--opacity", type=float, default=0.55)
    parser.add_argument("--max-points", type=int, default=None, help="Optional cap for faster CPU rendering")
    parser.add_argument("--no-viewer", action="store_true", help="Skip the interactive Pygame viewer")
    parser.add_argument("--window-scale", type=int, default=2, help="Scale factor for the Pygame viewer window")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        if args.local_dir:
            ...
            print(f"Using local sample: {data_dir}")
        else:
            data_dir = tmp
            print(f"Downloading sample from {args.s3_uri}")
            download_s3_prefix(args.s3_uri, data_dir)

        rgb = load_rgb(find_rgb_file(data_dir))
        ...
        # If we backproject at stride > 1, render at the strided resolution.
        render_w = w // args.stride
        render_h = h // args.stride
        fx_s = fx / args.stride
        fy_s = fy / args.stride
        cx_s = cx / args.stride
        cy_s = cy / args.stride
        rgb_ref = rgb[::args.stride, ::args.stride]

        gaussians = points_to_gaussians(
            points,
            colors,
            fx=fx_s,
            base_world_scale=args.base_world_scale,
            min_radius_px=args.min_radius_px,
            max_radius_px=args.max_radius_px,
            opacity=args.opacity,
        )

        np.savez_compressed(out_dir / "gaussians.npz", **gaussians)

        print("points / gaussians:", points.shape[0])
        print("render resolution:", render_w, "x", render_h)
        print("radius_px min/mean/max:",
              float(gaussians["radius_px"].min()),
              float(gaussians["radius_px"].mean()),
              float(gaussians["radius_px"].max()))
        print("depth range:",
              float(depth[depth > 0].min()),
              float(depth.max()))

        save_rgb(out_dir / "source_reference.png", rgb_ref)

        # 1. Original view render: should resemble source_reference.png.
        original = render_gaussian_splats(
            points,
            colors,
            fx_s,
            fy_s,
            ...
            width=render_w,
            height=render_h,
            radii_px=gaussians["radius_px"],
            opacities=gaussians["opacity"],
            max_points=args.max_points,
        )
        save_rgb(out_dir / "gaussian_original.png", original)

        print("original MSE:", float(np.mean((original - rgb_ref) ** 2)))
        print("original PSNR:", psnr(original, rgb_ref))

        # 2. Nearby translated novel view.
        shifted_right = transform_points_camera(points, tx=0.25, ty=0.0, tz=0.0)
        shifted_radii = compute_depth_based_radii(
            shifted_right,
            fx=fx_s,
            base_world_scale=args.base_world_scale,
            min_px=args.min_radius_px,
            max_px=args.max_radius_px,
        )
        shifted_render = render_gaussian_splats(
            shifted_right,
            colors,
            fx_s,
            fy_s,
            ...
            width=render_w,
            height=render_h,
            radii_px=shifted_radii,
            opacities=gaussians["opacity"],
            max_points=args.max_points,
        )
        save_rgb(out_dir / "gaussian_shift_right_tx0.25.png", shifted_render)

        # 3. Nearby rotation novel view.
        yaw_right = transform_points_camera(points, yaw_deg=5.0)
        yaw_radii = compute_depth_based_radii(
            yaw_right,
            fx=fx_s,
            base_world_scale=args.base_world_scale,
            min_px=args.min_radius_px,
        ...
        yaw_render = render_gaussian_splats(
            yaw_right,
            colors,
            fx_s,
            fy_s,
        ...
        save_rgb(out_dir / "gaussian_yaw_right_5deg.png", yaw_render)

        print(f"Saved outputs to: {out_dir.resolve()}")

        if not args.no_viewer:
            viewer = InteractiveSplatViewer(
                points=points,
                colors=colors,
                fx=fx_s,
                fy=fy_s,
                ...
                width=render_w,
                height=render_h,
                box_limits=(-0.35, 0.35, -0.20, 0.20, -0.35, 0.35),
                base_world_scale=args.base_world_scale,
                opacity=args.opacity,
                min_radius_px=args.min_radius_px,
                max_radius_px=args.max_radius_px,
